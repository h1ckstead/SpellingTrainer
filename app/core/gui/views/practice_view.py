...
class PracticePage(BaseView):
    def __init__(self, parent, controller, current_user, previous_page, session):
        ...
        self.title_text = Label(self, text=strings.PRACTICE_PAGE_TITLE, font=self.controller.title_font,
                                image=self.avatar, compound="left", padx=10)
        ...
        self.session_history_block.grid(row=2, column=2, columnspan=2, pady=(7, 20), sticky="w")
        self.definition_block.grid(row=1, column=2, columnspan=3, padx=(0, 90), sticky="e")
        self.back_to_main_btn.grid(row=3, column=1, columnspan=2, sticky="w")
        self.close_button.grid(row=3, column=2, sticky="e")
        self.finish_button.grid(row=3, column=3, sticky="e")
...
class SpellingTrainerBlock(BaseFrame):
    def __init__(self, parent, controller, current_user, session):
        ...
        self.spellcheck_btn = CTAButton(self, text=strings.CHECK, width=125, state="disabled",
                                        command=lambda: [self.perform_spellcheck(), self.word_entry.delete(0, 'end'),
                                                         self.new_word(), self.after(1000, self.say_word)])

        # Display widgets and content blocks on the page
        self.play_word_btn.grid(row=0, column=0, padx=10, pady=(10, 5), sticky="w")
        self.volume_slider.grid(row=0, column=1, columnspan=3, padx=(0, 40), sticky="w")
        self.grid_rowconfigure(1, minsize=29)  # placeholder for Br/Am spelling hint
        self.word_entry.grid(row=2, column=0, columnspan=3, padx=(10, 43), sticky="w")
        self.spellcheck_hint.grid(row=3, column=0, columnspan=3, padx=4, pady=(0, 10), sticky="w")
        self.spellcheck_btn.grid(row=4, column=0, columnspan=2, padx=10, pady=(0, 26), sticky="w")

    def create_spelling_hint(self):
        attention_img = CTkImage(Image.open('assets/attention.png'), size=(13, 13))
        return CTkLabel(self, text="American spelling", text_color="#bd9029", image=attention_img, compound="left",
    ...
    def on_entry_change(self):
        ...
        if stripped_text:
            self.spellcheck_btn.configure(state="normal", fg_color="#246ba3")
        else:
            self.spellcheck_btn.configure(state="disabled", fg_color="#565b5e")
    ...
    def show_green_message(self):
        ...
        validation_msg = CTkLabel(validation_container, text=strings.CORRECT, text_color="#02732d",
                                  font=CTkFont(weight="bold"), compound="left", padx=6,
    ...
    def show_red_message_and_correction(self):
        ...
        validation_msg = CTkLabel(validation_container, text=strings.INCORRECT, text_color="#f24726",
                                  font=CTkFont(weight="bold"), image=CTkImage(Image.open('assets/incorrect.png'),
                                                                              size=(15, 15)), compound="left", padx=6)
        validation_msg.pack(padx=4, pady=4)
        return validation_container
    ...
    def new_word(self):
        ...
        word = list(self.word_dict.keys())[0]
        if "Spelling" in self.word_dict[word].keys():
            if self.word_dict[word]["Spelling"] == "AmE":
                self.spellcheck_hint.configure(text="American spelling")
            elif self.word_dict[word]["Spelling"] == "BrE":
                self.spellcheck_hint.configure(text="British spelling")
...
class SessionHistoryBlock(BaseFrame):
    def __init__(self, parent, controller):
        ...
        self.times_to_spell_text.grid(row=0, column=1, columnspan=2, padx=(0, 15), sticky="e")
        ...
        for i, (status_label, correction_label, times_to_spell_label) in \
            ...
            status_label.grid(row=i, column=0, sticky="w", padx=10)
            correction_label.grid(row=i, column=1, sticky="w")
            times_to_spell_label.grid(row=i, column=2, sticky="w")
    ...
    def update_user_input(self, word_dict, user_word, status):
        ...
        self.times_to_spell.append(
            self.parent.current_user.dictionaries.vocabulary[list(word_dict.keys())[0]]['Times_to_spell'])
...
class DefinitionBlock(BaseFrame):
    def __init__(self, parent, controller):
        ...
        self.show_definition_btn.grid(row=0, column=0, columnspan=2, padx=(10, 0), pady=(10, 0), sticky="w")
        self.definition_hint.grid(row=0, column=1, columnspan=2, padx=(38, 0), pady=(10, 0))
        self.horizontal_line.grid(row=2, column=0, columnspan=3, padx=(10, 0), pady=(0, 10), sticky="nw")
        # self.pencil_icon.grid(row=1, column=2, rowspan=2, columnspan=2, padx=(55, 0), pady=(0, 8), sticky="s")
    ...
    def show_definition(self):
        ...
        self.definition_header.grid(row=1, column=0, padx=(10, 0), sticky="nw")
        self.definition_field.grid(row=3, column=0, rowspan=2, columnspan=3, padx=(8, 0), sticky="w")
        if self.scroll:
            self.scroll.grid(row=3, column=3, columnspan=2)
        self.show_definition_btn.configure(state="disabled", fg_color="#565b5e")
    ...
    def hide_definition(self, word_dict, user_word, status):  # TODO: Think how to do it without params
        ...
        self.show_definition_btn.after(500, lambda: self.show_definition_btn.configure(state="normal",
                                                                                       fg_color="#246ba3"))

    def update_definition(self, word_dict):
        self.definition_field.configure(state='normal')
        self.definition_field.delete("1.0", tk.END)
        self.show_definition_btn.configure(state="normal", fg_color="#246ba3")

        word = list(word_dict.keys())[0]
        if "definition" not in word_dict[word].keys() or word_dict[word]["definition"] is None:
            self.definition_field.insert(tk.END, f"{strings.NO_DEFINITION_FOUND}")
        else:
            for part_of_speech in word_dict[word]['definition'].keys():
                self.definition_field.insert(tk.END, f"{part_of_speech}:\n", "bold")
                for i, definition in enumerate(word_dict[word]['definition'][part_of_speech], start=1):
            ...
            self.definition_field.configure(state='disabled')
